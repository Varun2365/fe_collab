...
const FunnelEditorIndex = () => {
  ...
  const handleMoveStage = async (stageId, direction) => {
    ...
    // If the moved stage is now at position 0 (top), update indexPageId
    if (newIndex === 0) {
      const movedStage = newStages[0];
      // Get the pageId from the stage - it could be stage.id or stage.pageId
      const stagePageId = movedStage.pageId || movedStage.id;
  ...
  return (
    <div className="editor-container">
      ...
      <header className="editor-header">
        <div>
          <h1>Funnel: {contentData.name || 'Untitled Funnel'}</h1>
        ...
        <aside className="editor-sidebar">
          ...
          <ul className="stages-list">
            {stages.map((stage, index) => {
              ...
              <li 
                key={stage.id} 
                className={`stage-item ${activeStageId === stage.id ? 'active' : ''} ${isIndexPage ? 'index-page' : ''}`}
                onClick={() => { 
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flex: 1, width: '100%', position: 'relative' }}>
                  <div style={{ 
                    display: 'flex', 
                    alignItems: 'center', 
                    gap: '8px', 
